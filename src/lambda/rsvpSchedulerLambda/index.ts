...
const parseEventBody = (event: any) => {
  if (!event) return {};
...
const resolveSecret = (event: any, body: any) => {
  const headerSecret =
    event?.headers?.["x-scheduler-secret"] || event?.headers?.["X-Scheduler-Secret"];
...
export const handler = async (event: any) => {
  try {
    ...
    console.log("rsvpSchedulerLambda invoked", {
