...
async function prepareEmailAttachments(compressedIVUrl: string, eventName: string) {
...
async function prepareIVAttachment(eventId: string, eventName: string, originalIVUrl: string): Promise<string> {
  try {
    const response = await fetch(originalIVUrl);
    if (!response.ok) throw new Error(`Failed to fetch IV: ${response.statusText}`);
...
/* ------------------------------ */
async function safeInvoke(functionName: string, payload: any, retries = 3) {
      ...
      const batchResults = await Promise.allSettled(
        batch.map(async (guestData) => {
          try {
            ...
            /* ✅ 4. CONVERT SVG → PNG */
            /* ------------------------------ */
            const pngResult: any = await safeInvoke(PNG_CONVERT_LAMBDA!, {
              guestId: guest._id.toString(),
              eventId,
