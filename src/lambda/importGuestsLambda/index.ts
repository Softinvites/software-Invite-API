...
async function prepareEmailAttachments(compressedIVUrl: string, eventName: string) {
  try {
    ...
    return [{
      ...
      s3Key: s3Key, // This matches what your email Lambda expects
...
export const handler = async (event: any) => {
  ...
  try {
    ...
    const emailSuccesses = emailResults.filter(r => 
    ...
    return {
      ...
      body: JSON.stringify({
        ...
        failedGuests: failed.slice(0, 100), // Limit response size
