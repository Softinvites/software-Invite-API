...
export const handler = async (event) => {
    try {
        ...
        const results = await Promise.all(qrItems.map(async (item, index) => {
            try {
                const { guestId, guestName, tableNo, others } = item;
                // 1. Generate QR SVG with custom gradient
                const svg = generateQrSvg(guestId || `guest_${index}`, item.bgColor || "#FFFFFF", item.centerColor || "#000000", item.edgeColor || "#000000");
                // 2. Convert SVG → PNG
                const buffer = await sharp(Buffer.from(svg))
                    .png()
                    .toBuffer();
                // 3. Safe filename
                const filename = `qr-${safeName(guestName)}_${safeName(tableNo)}_${safeName(others)}_${guestId || index}.png`;
