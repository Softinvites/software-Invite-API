...
//     // Invoke Lambda
//     const lambdaResponse = await invokeLambda(process.env.IMPORT_LAMBDA_FUNCTION_NAME!, {
//       fileUrl,
//     });
...
//     res.status(201).json({
//       message: `Imported ${result.totalProcessed} guests`,
//       guests: result.guests,
//       successCount: result.successful,
//       failedCount: result.guests.length - result.successful,
//     });
//   } catch (error) {
...
//       res.status(lambdaResponse.statusCode).json({
...
//      // ✅ Save each successful guest to DB
//      const guestsToInsert = result.guests
...
//        email: r.value.email || "",
//        phone: r.value.phone || "",
//        message: r.value.message || "",          // Required field in your schema
//        others: r.value.others || "",            // Optional
//        TableNo: r.value.TableNo || "",          // Optional
//        eventId,
//        qrCodeUrl: r.value.qrCodeUrl || null,
//        qrCodeBgColor: r.value.qrCodeBgColor || "",
//        qrCodeCenterColor: r.value.qrCodeCenterColor || "",
//        qrCodeEdgeColor: r.value.qrCodeEdgeColor || "",
//      }));
...
//     // Upload the file to S3 for Lambda to access
//     const fileKey = `uploads/${Date.now()}_${req.file.originalname}`;
//     const fileUrl = await uploadToS3(req.file.buffer, fileKey, req.file.mimetype);
...
// if (!testFetch.ok) throw new Error("Lambda won't be able to access this file.");

//     // Call Lambda to parse file, generate QR codes, upload to S3
//     const lambdaResponse = await invokeLambda(process.env.IMPORT_LAMBDA_FUNCTION_NAME!, {
//       fileUrl,
//       eventId,
//     });

//     // Clean up uploaded file on S3 (optional)
//     await deleteFromS3(fileKey);

//     const result = JSON.parse(lambdaResponse.body || "{}");

// // Use 500 if lambdaResponse.statusCode is not a number
...
//     // Lambda returns guests with QR URLs and success status, save them in DB
//     // result.guests = [{ fullname, qrCodeUrl, success, ...otherFields }]

//     const guestsToSave = (result.guests || [])
//     .filter((g: any) => g.status === "fulfilled")
//     .map((g: any) => g.value); // extract the actual guest object

//     // Prepare DB docs - add eventId and any other necessary fields
// const guestsData = guestsToSave.map((guest: any) => ({
...
//     // Save guests to DB in bulk or one-by-one
//     await Guest.insertMany(guestsData);




//     res.status(201).json({
//       message: `Imported ${result.totalProcessed} guests, saved ${guestsData.length} to DB`,
//       guests: guestsData,
//       successCount: result.successful,
//       failedCount: result.failed,
//     });
...
//           await deleteFromS3(key);
//         } catch (error) {
//           console.error("Error deleting QR from S3:", error);
//         }
//       }
//     });
