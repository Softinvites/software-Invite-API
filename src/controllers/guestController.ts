  ...
  try {
    ...
    if (email) {
      try {
        ...
        await sendEmail(
          email, 
          `${eventName} - Invitation - Enable Images for QR Code`, // Enhanced subject line
          emailContent
        );
...
export const importGuests = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    await invokeLambda(
      ...
      { fileUrl, eventId, userEmail: req.body.userEmail }, 
      true 
    );
...
const qrItems = guests
  .map((guest) => {
    try {
      ...
      return {
        key: path,
        guestId: guest._id.toString(),                // Add guestId
        guestName: guest.fullname || "Guest",
        tableNo: guest.TableNo || "NoTable",
        others: guest.others || "-",                 // Optional
        qrCodeBgColor: guest.qrCodeBgColor || "255,255,255",        // Add color fields
