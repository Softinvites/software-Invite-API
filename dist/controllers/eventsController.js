...
const createEvent = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const { name, date, location, description, ivBase64, // base64 image string
         } = req.body;
        // ✅ Validate
        const { error } = utils_1.createEventSchema.validate(req.body);
        if (error) {
            return res.status(400).json({ message: "Validation error", error });
        }
        // ✅ Create Event
        const event = new eventmodel_1.Event({
            name,
            date,
            location,
            description,
