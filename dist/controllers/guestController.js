...
const addGuest = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
      ...
      `);
        ...
        const uploadResponse = yield uploadImage_1.cloudinary.uploader.upload(`data:image/svg+xml;base64,${Buffer.from(svg).toString("base64")}`, {
            folder: "qr_codes",
            public_id: `${firstName}_${lastName}_qr`,
            overwrite: true,
        });
...
const updateGuest = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        ...
        if (guest.qrCode) {
            const oldPublicId = (_a = guest.qrCode.match(/\/qr_codes\/([^/]+)\./)) === null || _a === void 0 ? void 0 : _a[1];
            if (oldPublicId) {
                yield uploadImage_1.cloudinary.uploader.destroy(`qr_codes/${oldPublicId}`);
            }
        }
        ...
        const bgColorHex = qrCodeBgColor
            ? (0, colorUtils_1.rgbToHex)(qrCodeBgColor)
            : guest.qrCodeBgColor;
        const centerColorHex = qrCodeCenterColor
            ? (0, colorUtils_1.rgbToHex)(qrCodeCenterColor)
            : guest.qrCodeCenterColor;
        const edgeColorHex = qrCodeEdgeColor
            ? (0, colorUtils_1.rgbToHex)(qrCodeEdgeColor)
            : guest.qrCodeEdgeColor;
        // ✅ Generate new QR code data
        const qrCodeData = `First Name: ${firstName || guest.firstName}\nLast Name: ${lastName || guest.lastName}\nEmail: ${email || guest.email}\nPhone: ${phone || guest.phone}\nEvent: ${eventName}\nDate: ${eventDate}\nLocation: ${eventLocation}`;
      ...
      `);
        ...
        const uploadResponse = yield new Promise((resolve, reject) => {
            const uploadStream = uploadImage_1.cloudinary.uploader.upload_stream({ resource_type: "image", folder: "qr_codes" }, (error, result) => {
                if (error) {
                    reject(error);
                }
                else {
                    resolve(result); // ✅ Ensure TypeScript knows the response structure
                }
            });
        ...
        const uploadResponse = yield new Promise((resolve, reject) => {
            const uploadStream = uploadImage_1.cloudinary.uploader.upload_stream({ resource_type: "image", folder: "qrcodes" }, (error, result) => {
                if (error) {
                    ...
                    resolve(result); // ✅ Explicitly cast to correct type
                }
            });
            const readableStream = new stream_1.default.PassThrough();
            readableStream.end(pngBuffer);
...
const downloadAllQRCodes = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        for (const guest of guests) {
            ...
            const pngBuffer = yield (0, sharp_1.default)(Buffer.from(qrSvg)).png().toBuffer();
